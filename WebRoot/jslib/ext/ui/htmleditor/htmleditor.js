...
Ext.ux.HTMLEditor = Ext.extend(Ext.form.HtmlEditor, {
    ...
    toolbarItems: ['fonts', 'allformats', 'allfontsizes', 'allcolors', 'allalignments', 'alllinks', 'alllists', 'sourceedit'],
    ...
    updateToolbar: function(){
        ...
        this.tb.items.each(function(item){
            if (item.queryState) {
                ...
                if (item.queryEnabled) {
                    item.setDisabled(!this.doc.queryCommandEnabled(item.itemId));
                }
                else 
                    if (item.xtype = "tbcombo" && item.queryValue) {
                    	console.log(this);
...
Ext.reg('htmleditorimage',Ext.ux.HTMLEditor);