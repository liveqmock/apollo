...
function findPosY(obj)
{
	...
	if (obj.offsetParent)
	{      
		while (obj.offsetParent&&obj.offsetParent.tagName!='BODY')
		{
...
function addClassName(el, sClassName) {
	    ...
	    editor=document.all("txtEdit");
       if(el.className!=null&&el.className=='clsCellStore')
       {   
	       
	        var tb1=el.offsetParent;
	        
	       if(tb1.tagName=='TABLE')
	       {
	        editor.style.position="absolute";
	        editor.style.top=tb1.offsetTop+el.offsetTop+35;
	        editor.style.left=tb1.offsetLeft+el.offsetLeft+10;
	      
		editor.style.width=Math.abs(el.clientWidth+2);
                editor.style.height=Math.abs(el.clientHeight);
			  
	        if(el.getAttribute("expression")!=null&&(eval(el.getAttribute("expression"))!=undefined))
	         {
				try{
	              editor.value=eval(el.getAttribute("expression"));
				}
				catch(e)
				 {}
	         }
            ...
            if(el.getAttribute("expression")!=null&&(eval(el.getAttribute("expression"))!=undefined))
	          {
				if(editor.value!='')
					{
					try{
					 eval(el.getAttribute("expression")+'=\''+editor.value+'\';');
					}
					catch(e)
				     {
				     }
