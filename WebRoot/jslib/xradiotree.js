...
/*
 * WebFXRadioTree class
 */

function WebFXRadioTree(sText, bChecked, checkValue, sAction, sBehavior, sIcon, sOpenIcon) {
	this.base = WebFXTree;
...
WebFXRadioTree.prototype.toString = function() {
	...
	str += "<input type=\"radio\"" +
		" name=\"tree-radio-button-name\"" +
        	" onClick=\"checkRadioTreeNode('" + this.id + "',checked)\" " +
		" class=\"tree-radio-button\"" +
		(this._checked ? " checked=\"checked\"" : "") +
		" value=\"" + this._checkValue + "\"" +
		" />";
	// end insert radio button
...
/*
 * 得到所有被选中的radiobutton值的数组，如果没有选中，返回length为0的数组
 */
WebFXRadioTree.prototype.getCheckedValue = function() {
	...
	for(var i = 0; i < this.childNodes.length; i++)
	{
		var checkedInChild = this.childNodes[i].getCheckedValue();
		if(checkedInChild.length > 0)
		{
			checkedValues = checkedInChild;
			break;
		}
	}
...
WebFXRadioTree.prototype.getChecked = function () {
	var divEl = document.getElementById(this.id);
	var inputEl = divEl.getElementsByTagName("INPUT")[0];
	return this._checked = inputEl.checked;
};

WebFXRadioTree.prototype.setChecked = function (bChecked) {
...
function WebFXRadioTreeItem(sText, bChecked, checkValue, sAction, eParent, sIcon, sOpenIcon) {
 ...
 * 得到所有被选中的radiobutton值的数组，如果没有选中，返回length为0的数组
 */
WebFXRadioTreeItem.prototype.getCheckedValue = function() {
...
/**
 * 点击radio button时触发此方法，这是缺省实现，可在jsp中重新定义该方法
