	...
	implements Function
{
					   ...
					   Object[] args) throws JavaScriptException
	{
		...
		//从args中去掉多余的参数
		Object [] temp = new Object[paramTypes.length];
	...
	static Object retryIllegalAccessInvoke(Method method, Object obj,
										   Object[] args,
										   IllegalAccessException illegalAccess) throws
	...
	static Member findFunctionBak(Member[] methodsOrCtors, Object[] args)
	{
		...
		for (int i = 0; i < methodsOrCtors.length; i++)
		{
			...
			if (bestFitTypes == null)
			{
				...
				for (j = 0; j < paramTypes.length; j++)
				{
					if (!NativeJavaObject.canConvert(args[j], paramTypes[j]))
					{
						if (debug)
						{
							printDebug("Rejecting (args can't convert) ",
									   member, args);
						}
						break;
					}
			...
			else
			{
				...
				if (preference == PREFERENCE_AMBIGUOUS)
				{
					if (debug)
					{
						printDebug("Deferring ", member, args);
				...
				else if (preference == PREFERENCE_FIRST_ARG)
				{
					if (debug)
					{
						printDebug("Substituting ", member, args);
					}
					bestFit = member;
					bestFitTypes = paramTypes;
				}
				else
				{
						...
						member.getDeclaringClass()))
					{
						...
						if (debug)
						{
							printDebug("Rejecting (overridden static)",
									   member, args);
						}
						bestFit = member;
						bestFitTypes = paramTypes;
					...
					else
					{
						if (debug)
						{
							printDebug("Rejecting ", member, args);
						}
					}
				}
			}
		...
		for (int i = ambiguousMethods.size() - 1; i >= 0; i--)
		{
			...
			else
			{
				if (debug)
				{
					printDebug("UNRESOLVED: ", member, args);
				}
			}
		}
	...
	/**
	 * script中方法的参数个数可以比方法定义中的参数多，最终匹配的方法是参数匹配数最多的那个方法
	 * Find the correct function to call given the set of methods
	 * or constructors and the arguments.
	 * If no function can be found to call, return null.
	 */
	static Member findFunction(Member[] methodsOrCtors, Object[] args)
	{
		...
		for (int i = 0; i < methodsOrCtors.length; i++)
		{
			...
			else
			{
				...
				if (paramTypes.length < args.length)
				{
					//输入的参数比方法定义中的参数多，从输入参数中得到实际使用的参数
					usedArgs = new Object[paramTypes.length];
					for (int j = 0; j < usedArgs.length; j++)
					{
						usedArgs[j] = args[j];
					}
				}

				//判断输入的参数是否能跟方法定义中的参数匹配
				int j;
				for (j = 0; j < paramTypes.length; j++)
				{
					if (!NativeJavaObject.canConvert(usedArgs[j], paramTypes[j]))
					{
						if (debug)
						{
							printDebug("Rejecting (args can't convert) ",
				...
				if (j == paramTypes.length)
				{
					if (bestFitTypes == null)
					{
						//以前尚未找到匹配的方法，将当前方法设置为匹配方法
						bestFit = member;
						bestFitTypes = paramTypes;
					}
					else
					{
						//以前已找到匹配的方法，判断那个方法最匹配
						if (paramTypes.length > bestFitTypes.length)
						{
							//参数多者较少者更匹配
							bestFit = member;
							bestFitTypes = paramTypes;
						}
						else if (paramTypes.length == bestFitTypes.length)
						{
							//参数个数相等时，参数类型最匹配的方法为最匹配方法
							int preference =
								NativeJavaMethod.preferSignature(usedArgs,
								paramTypes,
								bestFitTypes);
							if (preference == PREFERENCE_AMBIGUOUS)
							{
								if (debug)
								{
									printDebug("Deferring ", member, usedArgs);
									// add to "ambiguity list"
								}
								if (ambiguousMethods == null)
								{
							...
							else if (preference == PREFERENCE_FIRST_ARG)
							{
								if (debug)
								{
									printDebug("Substituting ", member, usedArgs);
								}
								bestFit = member;
								bestFitTypes = paramTypes;
							}
							else
							{
								...
								else
								{
									if (debug)
									{
										printDebug("Rejecting ", member, usedArgs);
									}
								}
							}
						}
